...
def sales_report_view(request):
    ...
    if export_format == "csv":
        ...
        response["Content-Disposition"] = (
            f'attachment; filename="sales_report_{filter_type}_{date_from}_to_{date_to}.csv"'
        )
        ...
        story.append(
            Paragraph(
                f"Period: {date_from.strftime('%d %b %Y')} to {date_to.strftime('%d %b %Y')} ({filter_type.capitalize()})",
                subtitle_style,
            )
        )
        ...
        response["Content-Disposition"] = (
            f'attachment; filename="sales_report_{filter_type}_{date_from}_to_{date_to}.pdf"'
        )
        response.write(pdf)
        return response
